...
execute if score @s Job matches 3 run tellraw @a ["",{"translate":"%1$sは%2$sを纏った！！！","color":"green","with":[{"selector":"@s"},{"translate":"狩人のオーラ","hoverEvent":{"action":"show_text","contents":{"translate":"範囲内のプレイヤーの周りにいる敵の移動速度・属性耐性・攻撃力が下がる。"}}}]}]
execute if score @s Job matches 4 run tellraw @a ["",{"translate":"%1$sは%2$sを纏った！！！","color":"green","with":[{"selector":"@s"},{"translate":"白魔導士のオーラ","hoverEvent":{"action":"show_text","contents":{"translate":"範囲内のプレイヤーに悪い効果・とても悪い効果がかからなくなり、スーパーデバリアの効果と再生がつく。"}}}]}]
